...
export default function CustomerMenuPage() {
  const { menuItems, categories, menuStyle, restaurantName } = useMenu();
  ...
  const generateWhatsAppMessage = () => {
    ...
    const message = `Hello ${restaurantName}! I'd like to place an order:\n\n${itemsList}\n\nTotal: $${totalPrice.toFixed(2)}\n\nThank you!`;
    return `https://wa.me/250780000000?text=${encodeURIComponent(message)}`;
  };
    ...
    >
      ...
      <header className="w-full sticky top-0 z-50 bg-surface/95 backdrop-blur-xl flex justify-between items-center px-6 h-20">
        <div className="flex flex-col">
          <Link href="/" className="text-2xl font-[var(--headline-font)] font-black tracking-tighter text-[var(--primary-color)]">MENUZA AI</Link>
          <span className="text-[10px] font-bold uppercase tracking-widest text-secondary -mt-1 font-[var(--body-font)]">{restaurantName}</span>
        </div>
        ...
        <div className="flex px-6 gap-3">
          {categories.map((cat) => (
            <button key={cat.id} onClick={() => setActiveCategory(cat.id)}
              className={`px-6 py-2.5 font-[var(--headline-font)] font-bold text-sm whitespace-nowrap transition-all duration-300 rounded-[var(--border-radius)] ${activeCategory === cat.id
      ...
      <main className="px-6 pt-4 space-y-8">
        ...
        <section className="space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="font-[var(--headline-font)] text-xl font-extrabold tracking-tight capitalize">{categories.find(c => c.id === activeCategory)?.name || activeCategory}</h3>
            <span className="text-[var(--primary-color)] font-[var(--headline-font)] text-xs font-bold uppercase tracking-widest">See All</span>
          </div>
              ...
              <div key={item.id} className="bg-surface-container-lowest overflow-hidden flex flex-col shadow-sm border border-outline-variant/10 rounded-[var(--border-radius)]">
                <div className="relative h-52">
                  ...
                  <div className="absolute top-4 right-4 bg-white/95 backdrop-blur-md px-4 py-2 z-10 shadow-sm rounded-[calc(var(--border-radius)/2)]">
                    <span className="font-[var(--headline-font)] font-extrabold text-lg text-[var(--primary-color)]">${item.price.toFixed(2)}</span>
                  </div>
                </div>
                <div className="p-6 flex flex-col">
                  <div className="flex justify-between items-start mb-2">
                    <h4 className="font-[var(--headline-font)] text-lg font-extrabold">{item.name}</h4>
                    {item.badge && (
                  ...
                  <p className="text-on-surface-variant text-sm leading-relaxed mb-6 font-medium opacity-80 font-[var(--body-font)]">{item.description}</p>
                  <button onClick={() => addToCart(item)}
                    className="w-full py-4 text-white font-[var(--headline-font)] font-bold flex items-center justify-center gap-2 active:scale-95 transition-all hover:opacity-90 bg-[var(--primary-color)] rounded-[var(--border-radius)] premium-shadow"
                  >
                    Add to Cart
        ...
        <div className="max-w-md mx-auto pointer-events-auto">
          {totalItems > 0 ? (
            ...
            >
              ...
              <span className="font-[var(--headline-font)] font-extrabold tracking-tight uppercase text-sm">
                Order via WhatsApp ({totalItems}) — ${totalPrice.toFixed(2)}
              </span>
            </a>
          ) : (
