  ...
  return (
    <div className="min-h-screen bg-surface text-on-surface pb-32">
      <header className="w-full sticky top-0 z-50 bg-surface/95 backdrop-blur-xl flex items-center px-6 h-16 gap-4">
        ...
        <h1 className="font-headline font-bold text-lg">Order Summary</h1>
      </header>
      ...
      <main className="px-6 py-8 max-w-lg mx-auto space-y-8">
        ...
        <div className="bg-surface-container-lowest rounded-[2rem] p-6 border border-surface-container/50">
          <h3 className="font-headline font-bold mb-4">Your Order</h3>
          <div className="space-y-4">
            {items.map((item) => (
              <div key={item.id} className="flex justify-between items-center">
                ...
                <span className="font-headline font-bold text-primary">{formatPrice(item.price * item.quantity, currency)}</span>
              </div>
            ))}
          </div>
          <div className="mt-6 pt-4 border-t border-surface-container flex justify-between items-center">
            ...
            <span className="font-headline font-extrabold text-2xl text-primary">{formatPrice(total, currency)}</span>
          </div>
        </div>

        {/* Customer Info */}
        <div className="bg-surface-container-lowest rounded-[2rem] p-6 border border-surface-container/50 space-y-4">
          <h3 className="font-headline font-bold mb-2">Your Details (Optional)</h3>
          <div>
        ...
        <div className="bg-surface-container-lowest rounded-[2rem] p-6 border border-surface-container/50">
          <h3 className="font-headline font-bold mb-3">WhatsApp Message Preview</h3>
      ...
      <div className="fixed bottom-0 left-0 w-full p-6 z-50">
        <div className="max-w-md mx-auto">
          <a href={whatsappUrl} target="_blank" rel="noopener noreferrer"
            className="w-full h-16 bg-whatsapp hover:bg-whatsapp-dark text-white rounded-full flex items-center justify-center gap-3 shadow-[0_12px_40px_rgba(37,211,102,0.4)] active:scale-95 transition-colors">
            ...
            <span className="font-headline font-extrabold tracking-tight uppercase text-sm">Order via WhatsApp</span>
          </a>
        </div>
      </div>
    </div>
