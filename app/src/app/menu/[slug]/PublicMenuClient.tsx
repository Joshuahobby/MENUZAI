...
export default function PublicMenuClient({
  ...
  const menuStyle: MenuStyle = { ...defaultStyle, ...style };
              ...
              <div key={item.id} className={`bg-surface-container-lowest overflow-hidden flex flex-col shadow-sm border border-outline-variant/10 rounded-[var(--border-radius)] relative ${item.available === false ? "opacity-60" : ""}`}>
                ...
                <div className="p-6 flex flex-col">
                  ...
                  <button
                    ...
                    className={`w-full py-4 text-white font-[var(--font-headline)] font-bold flex items-center justify-center gap-2 active:scale-95 transition-all hover:opacity-90 bg-[var(--primary-color)] rounded-[var(--border-radius)] premium-shadow disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100`}
                  >
          ...
          {totalItems > 0 ? (
            ...
            >
              ...
              <span className="font-[var(--font-headline)] font-extrabold tracking-tight uppercase text-sm">
                Review Order ({totalItems}) — ${totalPrice.toFixed(2)}
              </span>
            </button>
          ) : (
